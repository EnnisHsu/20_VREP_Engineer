...
uint8_t CChassis::speed_control()
{
  ...
  */
  for (uint8_t k = 0; k < WHEEL_NUM; ++k )
     wheel_Out[k] = _Chassis_Constrain((int32_t)wheel_Out[k], (int32_t)-Param.MAX_MOTOR_OUTPUT, (int32_t)Param.MAX_MOTOR_OUTPUT);
  //cout << wheel_Out[0] << "A__" << wheel_Out[1] << "A__" << wheel_Out[2] << "A__" << wheel_Out[3] << "A__" << wheel_Out[4] << "A__" << wheel_Out[5] << endl;
  return 1;
}
...
inline void CChassis::model_resolve()
{
  ...
  #ifdef LUNAR_CHASSIS
  static const float z_scale = 0.63;
